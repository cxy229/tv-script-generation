...
def token_lookup():
    ...
    """
    # TODO: Implement Function
    punc_to_token = {'.':'||period||', ',':'||comma||', '"':'||quotation_mark||', ';':'||semicolon||',                     '!':'||exclamation_mark||', '?':'||question_mark||', '(':'||left_parentheses||',                     ')':'||right_parentheses||', '--':'||dash||', '\n':'||return||'}
...
helper.preprocess_and_save_data(data_dir, token_lookup, create_lookup_tables) # 先把token中的标点符号替换，再把word转换为int
...
# LSTM层数怎么得到？  自己加上
def get_init_cell(batch_size, rnn_size, keep_prob=0.8, lstm_layers=2):
    """
...
def get_batches(int_text, batch_size, seq_length):
    ...
    x_data = np.array(int_text)
    y_data = np.array(int_text[1:]+[int_text[0]])
    
    x_batches = np.split(x_data.reshape((batch_size, -1)), n_batches, axis=1)
    y_batches = np.split(y_data.reshape((batch_size, -1)), n_batches, axis=1)
